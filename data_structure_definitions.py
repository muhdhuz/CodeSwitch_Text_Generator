...
class insertionalModelStructure(object):
    ...
    def __init__(self):
        ...
        ## For Noun words only
        #self.switchLabels = ['NN', 'NNS', 'NNP', 'NNPS']
        ## For Adj words only
        #self.switchLabels = ["JJ", "JJR", "JJS"]
        ## For both Noun words and Adj words 
        #self.switchLabels = ['NN', 'NNS', 'NNP', 'NNPS',"JJ", "JJR", "JJS"]
...
class dfa(object):
    ...
    def mergeSpokes(self, linguistic_theory):
        if self.engEnd != -1:
            if self.hinEnd != -1:
                if self.mixEnd != -1:  # 1-1-1
                    self.mergeStates(self.engEnd, [self.mixEnd, self.hinEnd], linguistic_theory)
                    self.mixEnd = self.engEnd
                    self.hinEnd = self.engEnd
                else:  # 1-1-0
                    self.mergeStates(self.engEnd, [self.hinEnd], linguistic_theory)
                    self.hinEnd = self.engEnd
            else:
                if self.mixEnd != -1:  # 1-0-1
                    self.mergeStates(self.engEnd, [self.mixEnd], linguistic_theory)
                    self.mixEnd = self.engEnd
                else:  # 1-0-0
                    return
        else:
            if self.hinEnd != -1:
                if self.mixEnd != -1:  # 0-1-1
                    self.mergeStates(self.hinEnd, [self.mixEnd], linguistic_theory)
    ...
    def mergeStates(self, s1, others, linguistic_theory):
        temp = {}
    ...
    def mergeDfa(self, other, linguistic_theory):
    ...
    def mergeDfaSeries(self, other, linguistic_theory):
        ...
        self.mergeDfa(other, linguistic_theory)
        self.engEnd = other.engEnd
        self.hinEnd = other.hinEnd
        self.mixEnd = other.mixEnd

        if currFinal != -1:
            self.mergeStates(currFinal, [nextStart], linguistic_theory)

    def mergeDfaParallel(self, other, linguistic_theory):

        currFinal = self.finalStates[0]
        currStart = self.initialStates[0]
        nextStart = self.nextIndex
        self.mergeDfa(other, linguistic_theory)
        self.mergeStates(currStart, [nextStart], linguistic_theory)
        self.mergeStates(currFinal, [self.nextIndex - 1], linguistic_theory)
